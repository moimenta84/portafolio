...
import { Briefcase, ExternalLink, Trash2, RefreshCw, Plus, X, ChevronDown, FileText, MapPin, Building2, Zap } from "lucide-react";
...
type Status = "nueva" | "aplicada" | "entrevista" | "oferta" | "rechazada";
...
export default function JobKanban() {
  ...
  const load = async () => {
    ...
    const jobsData = await jRes.json() as Job[];
    const statsData = await sRes.json();
    setJobs(jobsData.map(j => ({ ...j, tech: typeof j.tech === "string" ? j.tech : JSON.stringify(j.tech) })));
    setStats(statsData);
    setLoading(false);
  };
  ...
  const updateStatus = async (id: number, status: Status) => {
    ...
    setJobs(prev => prev.map(j => j.id === id ? { ...j, status, applied_at: status === "aplicada" ? new Date().toISOString() : j.applied_at } : j));
  ...
  const scrapeNow = async () => {
    ...
    await load();
    setScraping(false);
    alert(`✅ Scraping completado\nNuevas ofertas: ${data.new_offers}\nAuto-aplicadas: ${data.auto_applied}`);
  };
  ...
  const techArr = (raw: string): string[] => {
    ...
    <div className="flex flex-col gap-4">
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <div className="bg-white/[0.04] rounded-xl p-3 border border-white/8">
            <div className="text-xs text-white/40">Match medio</div>
            <div className={`text-xl font-bold ${scoreColor(stats.avg_score)}`}>{stats.avg_score}%</div>
          </div>
          <div className="bg-white/[0.04] rounded-xl p-3 border border-white/8">
            <div className="text-xs text-white/40">Aplicadas</div>
            <div className="text-xl font-bold text-yellow-400">{stats.by_status.find(s => s.status === "aplicada")?.count ?? 0}</div>
          </div>
          <div className="bg-white/[0.04] rounded-xl p-3 border border-white/8">
            <div className="text-xs text-white/40">Entrevistas</div>
            <div className="text-xl font-bold text-purple-400">{stats.by_status.find(s => s.status === "entrevista")?.count ?? 0}</div>
          </div>
        </div>
      )}
      ...
      {addOpen && (
        <div className="bg-white/[0.04] rounded-xl p-4 border border-white/10 flex flex-col gap-2">
          ...
          <div className="flex gap-2 mt-1">
            ...
            <button onClick={() => setAddOpen(false)} className="px-3 py-1.5 rounded-lg bg-white/5 text-white/50 text-xs border border-white/10 hover:bg-white/10 transition-all">Cancelar</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3 overflow-x-auto">
        {COLUMNS.map(col => {
          ...
          return (
            <div key={col.key} className={`flex flex-col gap-2 rounded-xl p-2 border min-h-[200px] ${col.bg}`}>
              <div className="flex items-center justify-between px-1">
                <span className={`text-[11px] font-bold uppercase tracking-wider ${col.color}`}>{col.label}</span>
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-white/5 ${col.color}`}>{colJobs.length}</span>
              </div>
              ...
              {colJobs.map(job => (
                <div
                  ...
                  className="bg-white/[0.05] hover:bg-white/[0.09] border border-white/8 rounded-lg p-2.5 cursor-pointer transition-all group"
                >
                  <div className="text-[11px] font-semibold text-white leading-tight line-clamp-2">{job.title}</div>
                  <div className="flex items-center gap-1 mt-1">
                    <Building2 size={9} className="text-white/30 shrink-0" />
                    <span className="text-[10px] text-white/40 truncate">{job.company}</span>
                  </div>
                  <div className="flex items-center justify-between mt-1.5">
                    <span className={`text-[10px] font-bold ${scoreColor(job.match_score)}`}>{job.match_score}%</span>
                    <span className="text-[9px] text-white/25 font-mono">{job.source}</span>
                  </div>
                  {techArr(job.tech).slice(0, 2).length > 0 && (
                    <div className="flex gap-1 mt-1.5 flex-wrap">
                      {techArr(job.tech).slice(0, 2).map(t => (
                        <span key={t} className="text-[9px] px-1.5 py-0.5 rounded-full bg-white/8 text-white/40 font-mono">{t}</span>
                      ))}
                    </div>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setSelected(null)}>
          <div className="bg-[#0d1117] border border-white/12 rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto p-5 flex flex-col gap-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={() => setSelected(null)} className="text-white/30 hover:text-white/70"><X size={16} /></button>
            </div>

            <div className="flex items-center gap-2 flex-wrap">
              <span className={`text-xs font-bold ${scoreColor(selected.match_score)}`}>Match: {selected.match_score}%</span>
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[11px] text-white/40 font-semibold uppercase tracking-wider">Estado</span>
            ...
            {selected.cover_letter && (
              <div className="flex flex-col gap-1.5">
                <span className="text-[11px] text-white/40 font-semibold uppercase tracking-wider flex items-center gap-1"><FileText size={10} />Carta generada</span>
                <p className="text-[11px] text-white/60 leading-relaxed bg-white/[0.03] rounded-lg p-3 border border-white/8">{selected.cover_letter}</p>
              </div>
            )}
